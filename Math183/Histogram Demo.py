    ...
    def construct(self):
        ...
        def animateBinStrategy(costTex, hist, bins, timings, vals, fade=True):
            ...
            for x in bins:
                ...
                newlabel = MathTex(str(x), font_size=18, color=BLUE).next_to(newtick, DOWN, SMALL_BUFF)
                binning.add(newtick, newlabel)
                self.play(FadeIn(newtick, newlabel), run_time=0.5)

            self.wait(1)
            brace = Brace(binning, DOWN, color=YELLOW)
            braceText = MathTex(r"\text{Binning Strategy}", font_size=20, color=YELLOW).next_to(brace, DOWN, SMALL_BUFF)
            self.play(FadeIn(brace, braceText))
            self.wait(3)
            self.play(FadeOut(brace, braceText))

            rect = SurroundingRectangle(costTex, color=YELLOW)
            self.play(Create(rect), run_time=2)
            unit = hist.get_unit_size()
            binwidth = (bins[1] - bins[0]) * unit
            sq = Rectangle(width=binwidth, height=unit, color=YELLOW, fill_color=YELLOW, fill_opacity=0.3)
            ...
            for (val, t) in zip(vals, timings):
                ...
                newrect = SurroundingRectangle(val, color=YELLOW)
                self.play(Transform(rect, newrect), run_time=t)
                self.wait(t)
                valcopy = val.copy()
                self.add(valcopy)
                self.play(valcopy.animate.next_to(loc, DOWN).set_color(YELLOW), run_time=t)
                self.wait(t)
            ...
            yaxis = NumberLine(x_range=[0, 5, 1], color=BLUE, include_tip=False, unit_size=unit,
                               label_direction=0.5 * UL, rotation=90 * DEGREES, include_numbers=True,
                               font_size=18).move_to(fauxorigin, aligned_edge=DOWN)
            yaxis.numbers.set_color(BLUE)
            ylabel = MathTex(r"\text{Counts}", font_size=20, color=BLUE).rotate(angle=90 * DEGREES).next_to(yaxis, LEFT)
        ...
        costTex = MathTex(r"\underline{\text{Recent Chipotle Bills}}", font_size=24)
        vals = [DecimalNumber(x, font_size=24) for x in costs]
        costsGroup = VGroup(costTex, *vals).arrange(DOWN).to_edge(LEFT, buff=MED_LARGE_BUFF)

        intro = MathTex(r"\text{Visualizing Numeric Data:  The Histogram", font_size=48).shift(UP)
        hist = NumberLine(x_range=[5.5, 15.5, 1], length=10, color=BLUE,
                          include_tip=False, include_ticks=False).to_edge(RIGHT, buff=LARGE_BUFF).shift(2 * DOWN)
        histlabel = MathTex(r"\text{cost}", font_size=20, color=BLUE).next_to(hist, RIGHT)
        skip = False
