...
class ITViz(Scene):
    def construct(self):
        ...
        question = MathTex(r"\text{Visual Intuition Behind}\\",
                           r"\text{The Integral Test for Series}").arrange(DOWN)
        series = MathTex(r"a_1", r"+", r"a_2", r"+", r"a_3", r"+", r"a_4", r"+", r"a_5", r"+", r"a_6", r"+",
                         r"\cdots").to_edge(UP, MED_SMALL_BUFF)
        maxterm = 6
        ...
        dots = MathTex(r"\cdots", color=WHITE, font_size=44).move_to(axes.c2p(6.3, f(7) / 2))
        ...
        flabel = MathTex(r"f(x)", font_size=28).next_to(axes.c2p(0.9, f(0.9)), LEFT, MED_SMALL_BUFF)
        self.play(FadeIn(graph, flabel), run_time=2)
        self.wait(4)

        area = axes.get_area(graph, x_range=[1, 6.5], color=WHITE)
        comparison = MathTex(r"\int_1^{\infty} f(x) \, dx", r"<", r"\text{series}",
                             r"\text{If divergent}", r"\Rightarrow", r"\text{divergent}",
